...
// Ver2.8.8 プレイヤー名方式（Google名は画面に出さない）
...
window.loadFriendData = async function(friendId){
  ...
  if(code && code.length === 8){
    ...
    if(codeSnap.exists()){
      ...
      const uid = data.uid;
      const profileSnap = uid ? await getDoc(doc(db,"userProfiles",uid)) : null;
      const publicSnap = await getDoc(doc(db,"players",code));
      if(publicSnap.exists()) return publicSnap.data();
      if(profileSnap && profileSnap.exists()){
        const p = profileSnap.data();
        return {
          playerId:"google_"+uid,
          friendCode:code,
          name:p.playerName || "名無し",
          playerName:p.playerName || "名無し",
          icon:p.icon || "",
          title:p.title || "初心者",
          level:p.level || 1,
          bestRandomScore:p.bestRandomScore || 0
        };
      }
    }
  }
...
window.loadCloudPlayerDataAndApply = async function(){
  ...
  // 初回ログイン：クラウドにデータがなければ、この端末のデータを自動保存
  if(!snap.exists()){
